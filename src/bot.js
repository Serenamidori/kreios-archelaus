		...
		if (message.content.substring(0, 1) == '!') {
			...
			switch(cmd) {
				...
				case 'dc':
					if (args.length == 0) {
						...
						if (length === 2 || length === 1) {
							difficulty = args[0];
						} else { // TODO: This needs to handle both one word and two word entries
							difficulty = args.slice(0, 2).join(' ');
						}
						// TODO: eventually allow the rest of the message to be included in the successful response
						dc = calculations.calcDC(difficulty)

						if (dc < 0) {
							message.reply('didn\'t quite understand what your __difficulty rating__ is. Try one of the following options:\n**ve** (very easy), **e** (easy), **m** (medium), **h** (hard), **vh** (very hard), **ni** (nearly impossible)');
						} else {
							message.reply('you must succeed on a DC ' + dc + ' for this check.');
						}
					}
					break;
				case 'difficulty':
				...
				case 'd':
					if (args.length == 0) {
						...
						var roll = rollTable[random.rand(6)-1];
						message.reply('for this ' + guess + ' situation the difficulty is **' + difficultiesNames[roll] + '** (DC ' + calculations.calcDC(difficultiesCodes[roll], true) +').');
					}
					break;
				case 'setCampaign':
				case 'setcampaign':
				...
				case 'npc':
					...
					if (args[0] == 'build' || args[0] == 'b') {
						...
						npc += "```\n Race: " + raceArray[random.rand(raceArray.length)-1];

						if (contains(args, 'class') || contains(args, 'c')) {
							npc += "\n Class: " + npcBuild[3][random.rand(npcBuild[3].length)-1];
						}  
						//check for other args, otherwise default to only Race, Gender, Eye Color, Hair Color and Length, Height, Weight, and 1 Misc.
						npc += "\n Gender: " + npcBuild[4][random.rand(npcBuild[4].length)-1] + 
							"\n Hair Color: " + npcBuild[5][random.rand(npcBuild[5].length)-1] + 
							"\n Hair Length: " + npcBuild[6][random.rand(npcBuild[6].length)-1] + 
							"\n Eye Color: " + npcBuild[5][random.rand(npcBuild[5].length)-1] + 
							"\n Height: " + npcBuild[7][random.rand(npcBuild[7].length)-1] + 
							"\n Weight: " + npcBuild[8][random.rand(npcBuild[8].length)-1] + 
							"\n Misc. Trait: " + npcBuild[9][random.rand(npcBuild[9].length)-1];
					...
					if (args[0] == 'attitude'|| args[0] == 'a') {
						message.reply(constants.npcAttitude[random.rand(3)-1]);
					}
					break;
				case 'o':
				case 'oracle':
					CampaignModel.findOne({ channelId: message.channel.id, }).then(campaign => {
						...
						if (campaign) {
							...
							if (ips >= 3) {
								message.reply(responses[random.rand(responses.length-1)] + ' Intervention:** ' + interventions[random.rand(6)-1]);
								ips = 0;
							}
				...
				case 'p':
				case 'portent':
					var index1 = random.rand(3)-1;
					var index2 = random.rand(3)-1;
					var word1 = portents[index1][random.rand(portents[index1].length)-1];
					var word2 = portents[index2][random.rand(portents[index2].length)-1];

					message.reply(constants.portents[random.rand(constants.portents.length-1)] + ': "' + word1 + '" and "' + word2 + '"');
					break;
				case 'twene':
					message.reply('Ah, so things are not as expected? ' + constants.twene[random.rand(constants.twene.length-1)]);
					break;
				case 'flip':
					var coin = random.rand(2) == 2 ? "heads" : "tails";
				...
				case 'kroll':
					...
					message.reply(messageHandler.diceMessage(index, bonus, symbol, count, numbers, sides, total));
					break;
			...
			if (message.mentions.has(bot.user)) {
				...
				if (contains(words, 'daddy')) {
					...
					message.reply(responses[random.rand(responses.length - 1)]);
					return;
				}
				// compliments

  ...
  if (message.content.startsWith("!")) {
    ...
    if (message.mentions.has(bot.user)) {
      ...
      if (contains(words, "daddy")) {
        var responses = ["**no.** None of that.", "*absolutely not*."];
        message.reply(responses[utils.random.rand(responses.length - 1)]);
        return;
      }
      // compliments
      if (
        contains(words, "cute") ||
        contains(words, "adorable") ||
        contains(words, "handsome") ||
        contains(words, "hot") ||
        contains(words, "sexy")
      ) {
        var responses = [
      ...
      if (contains(words, "fight me")) {
        var responses = [
          "alright, I use my Sacred DM Dragon's Flame. It instantly takes out all your remaining HP and kills you super dead. Want to try that again?",
          "meet me in the Denny's parking lot.",
          "you _really_ want to fight the Dungeon Master? _Pff_.",
        ];
        message.reply(responses[utils.random.rand(responses.length - 1)]);
        return;
      }
      ...
      if (contains(words, "fuck you") || contains(words, "fuck off")) {
        var responses = [
          "_language._ You kiss your mother with that mouth?",
          "is that an invitation? Oh, no that's you trying to be clever.",
          "no thank you.",
          "you roll a natural 1 and die, roll a new character.",
        ];
        message.reply(responses[utils.random.rand(responses.length - 1)]);
        return;
      }
      ...
      if (contains(words, "fuck me")) {
        var responses = [
          "my, how forward!",
          "oh, I'm sure that will go well for you.",
          "what, like killing your character? I could do that~",
        ];
        message.reply(responses[utils.random.rand(responses.length - 1)]);
        return;
      }
      // thank you
      if (
        contains(words, "thank you") ||
        contains(words, "thank") ||
        contains(words, "thanks")
      ) {
        var responses = [
          "you're most welcome.",
          "but of course.",
      ...
      if (
        message.author.id == 108393182563213312 ||
        message.author.id == 146434211912351744 ||
        message.author.id == 302942448714383360
      ) {
        var responses = [
          "why hello there~",
          "_mmmmyes~?_",
      ...
      var responses = [
        ...
        "what do you want?",
        "may I help you?",
        "yes, I was paying attention, what?",
        "_mhmm?_",
        "you rang?",
